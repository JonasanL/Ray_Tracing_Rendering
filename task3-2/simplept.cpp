...
Vec radiance_half(const Ray &r, int depth, bool flag, int id){
    //find current object
    ...
    if (rng() < p){
        ...
        const Sphere &nextobj = spheres[nextid];
        const BRDF &nextbrdf = nextobj.brdf;
        if (nextbrdf.isSpecular() == true){
        rad = rad + (radiance_half(Ray(y,InvertI),depth+1,flag,nextid).mult(brdf.eval(n,o,i)) * n.dot(i))*(1/(p*pdfI));
        }
        else{
            rad = rad + (radiance(Ray(y,InvertI),depth+1,flag,nextid).mult(brdf.eval(n,o,i)) * n.dot(i))*(1/(p*pdfI));
        }
        if (nextid == 7 && brdf.isSpecular() == true){
            rad = spheres[7].e;
        }
    }
    return rad;
...
Vec receivedRadiance(const Ray &r, int depth, bool flag) {
    ...
    //RayTracing
    //printf("%f",a.x);
    if (brdf.isSpecular() == true){
        return obj.e + radiance_half(Ray(x,o),depth,flag,id);
    }
